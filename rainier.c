...
void init()
{
	...
	get_mesh()->point_to_compute = NULL;
	get_mesh()->boundary_point_to_compute = NULL;
...
int main(int argc, char const *argv[])
{
	...
	printf("cells :%d %d\n",get_mesh()->faces[0].to_cell[0],get_mesh()->faces[0].to_cell[1]);
	int i;
	for(i=0;i<NB_BAND;i++)
	{
		printf("%f %f\n",get_mesh()->ckmodel.fQH_store[i],get_mesh()->ckmodel.fQC_store[i]);
	}
